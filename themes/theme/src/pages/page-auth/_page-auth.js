        ...
        <input
          ...
          class={`
            ${form_validation_errors[name].error 
              ? 
                'block w-full rounded-md border-red-300 pr-10 text-red-900 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500 sm:text-sm shadow-sm' 
              :
      ...
      updateNotification({
        ...
        onClose: () => {
          ...
          if (data.user.roles.includes('administrator'))
            window.location.href = `${PHP.site_url}/admin-dashboard`;
          else
            window.location.href = `${PHP.site_url}/orders`;
    ...
    <>
      ...
      class="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
          ...
          <img class="mx-auto h-14 w-auto" src={logo} alt="Logo" />
          
          <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">
            { auth_type === 'register' && 'Register a new account'}
            { auth_type === 'login' && 'Login to your account'}
          </h2>
