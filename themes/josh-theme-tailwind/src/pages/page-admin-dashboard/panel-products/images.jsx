...
const Images = ({ formChangeHandler, closeModal }) => {
  ...
  useEffect(() => {
    ...
    const getImages = async () => {     
      // const url = 'http://jadefse.local/wp-json/wp/v2/media';
      const url = `${PHP.wp_rest_url}/media`;
      const images = await fetchGET(url);

      const { media_details, title } = images;
      // const name = title.rendered;

      console.log('images: ', images);
      setImages(() => images.map(({ id, source_url, media_details, title }) => ({
        id,
        source_url,
        // title: title.rendered,
        title: media_details.file.split('/')[2],
        filesize: media_details.filesize,
            ...
            <div class="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
              ...
              <section class="mt-8 pb-16" aria-labelledby="gallery-heading">
                ...
                <ul role="list" class="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                  ...
                  { images.length > 0 && images.map(({ id, source_url, title, filesize, sizes }) => (
                    <li // image card
                      onClick={() => {

                        // URL for large version
                        const { height, width, source_url } = sizes.full;

                        setActiveIdx(id); // for UI-ring
                        formChangeHandler({ target: { type: 'image', name: 'image', image_id: id, image_url: source_url, image_height: height, image_width: width }}); // form
                      }}
                      class="relative"
                    >
                      ...
                      <div 
                        class={`
                          ${id === active_idx ? 'ring-2 ring-offset-2 ring-indigo-500' : ''} 
                          group block w-full aspect-w-10 aspect-h-7 rounded-lg bg-gray-100 overflow-hidden
                        `}>
                        {/* <!-- Current: "", Default: "group-hover:opacity-75" --> */}
                        {/* <img src="https://images.unsplash.com/photo-1582053433976-25c00369fc93?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=512&q=80" alt="" class="object-cover pointer-events-none" /> */}
                        <img src={source_url} alt="" class="object-cover pointer-events-none" />
                        <button type="button" class="absolute inset-0 focus:outline-none">
                          <span class="sr-only">View details for IMG_4985.HEIC</span>
                        </button>
                      </div>
                      <p class="pointer-events-none mt-2 block truncate text-sm font-medium text-gray-900">{title}</p>
                      <p class="pointer-events-none block text-sm font-medium text-gray-500">{ toMB(filesize) } MB</p>
                    </li>                 
                  ))}
            ...
            <div class="space-y-6 pb-16">
              ...
              <div class="flex">
                <button 
                  ...
                  disabled={active_idx === undefined}
                  type="button" 
                  style={ active_idx === undefined ? { opacity: 0.5 } : {} }
                  class={`
                    flex-1  rounded-md  border  border-transparent 
                    bg-indigo-600  text-white  shadow-sm 
                    py-2  px-4  text-sm  font-medium 
                    ${active_idx !== undefined ? 'hover:bg-indigo-700 focus:outline-none  focus:ring-2  focus:ring-indigo-500  focus:ring-offset-2' : ''}
                  `}
                >
                    Choose
                </button>
                <button 
                  onClick={() => {
                    setActiveIdx(undefined); // for UI-ring
                    formChangeHandler({ target: { type: 'image', name: 'image', image_id: null, image_url: null, image_height: null, image_width: null }}); // form
                    closeModal();
                  }}
                  type="button" 
